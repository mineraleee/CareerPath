...
export default function AssessmentPage() {
  ...
  if (current < questions.length - 1) {
    ...
    // Simpan ke localStorage supaya bisa dibaca di /dashboard
    localStorage.setItem(`assessment_${email}`, JSON.stringify(payload));

    // (Opsional) Kirim ke API (kalau mau)
    axios
      .post("https://career-path-api.onrender.com/api/assess-career", payload)
      .then((res) => {
        console.log("Assessment result:", res.data);
        router.push("/chooseCareer");
      })
      .catch((err) => {
        console.error("Submission failed:", err);
        router.push("/chooseCareer"); // tetap lanjut walau gagal kirim
      });
  }
};
