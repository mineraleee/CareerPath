...
export default function CourseCard({ course }: { course: Course }) {
    ...
    axios
      ...
      .then((res) => {
        const isSaved = res.data.some(
          (item: any) => item.course_id === course.course_id
        );
        setSaved(isSaved);
      })
      .catch((err) => {
    ...
    axios
      .post(
        'https://career-path-api.onrender.com/api/courses/save',
        {
          ...
          course_url: '#', // karena tidak tersedia, kamu bisa atur nanti
        },
        {
          headers: {
            Authorization: `Bearer ${userToken}`,
  ...
  return (
    <div className="bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden border border-gray-100 relative">
      ...
      >
        {saved ? (
          <BookmarkCheck className="w-5 h-5 text-green-300" />
        ) : (
      ...
      <div className="p-4 flex flex-col gap-2">
        ...
        <button
          disabled
          className="mt-4 bg-[#0EA5E9] text-white text-sm font-medium py-2 rounded-md text-center cursor-not-allowed opacity-60"
        >
